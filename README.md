...
**分析结果**
- redshift在3g及3g以上数据量情况下，碾压mysql和postgresql，越大数据量，优势更加明显。而在1g的数据量下，mysql和postgresql更加有优势
...
- mysql和postgresql在db.r4.large的硬件配置下，由于太慢，无法完成10g数据量测试。而在db.r4.xlarge的配置下，可以完成查询。而在监控端，发现其实cpu占用率不是很高，这说明内存对于数据性能的提高非常关键。
